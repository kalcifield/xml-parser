...
def reset_file():
    file_name = "presql/pre_freedata_offer.sql"
    open(file_name, "w").close()
    file_name = "presql/pre_freedata_offer_nestedkeyval.sql"
    open(file_name, "w").close()
...
def parse_xml_to_json():
    print("freedata_offer")
    version = "1.0.0"
    # objecttype -> Offers/Addons/Rewards etc..
    # object_uid = objectType
    object_type = "FreeDataOffers"
    value_type = "Offer"
    # print(parser['OfferConfig'][object_type][value_type]) # list of keyvalues
    freedataoffer_list = parser['OfferConfig'][object_type][value_type]
    for offer in freedataoffer_list:
        offer_id = offer['Id']
        object_uid = offer_id + "-" + version
        implemented_structure_uid = "Pre-FreeDataOffer-1.0.0"
        write_to_file("object", object_uid, offer_id, implemented_structure_uid)
        for key, val in offer.items():
            keyvalue_uid = offer_id + "-" + version + "-" + key
            implementedrow_uid = "Pre-FreeDataOffer-"+version + "-" + key
            input_val = "null"
            ...
            write_to_file("key_value", keyvalue_uid,offer_id ,input_val, implementedrow_uid,object_uid)
            gc.collect()
